...
export default function Navbar({ onOpenBookingModal }) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 transition-all duration-300">
      ...
      <div 
        className={`text-[10px] sm:text-xs py-1.5 px-3 sm:px-4 text-center border-b flex items-center justify-center gap-1.5 sm:gap-2 font-medium transition-colors ${
          isDark 
      ...
      >
        ...
        <span className="truncate max-w-[260px] sm:max-w-none">
          2025–2026 Season Bookings Open • Special Lawn & Ballroom Packages
        </span>
      ...
      <nav 
        className={`transition-all duration-300 px-3 sm:px-6 lg:px-8 ${
          isDark
            ? scrolled 
              ? 'py-2.5 bg-[#060D17]/95 backdrop-blur-md shadow-2xl border-b border-[#D4AF37]/30' 
              : 'py-3 sm:py-4 bg-[#060D17]/85 backdrop-blur-sm border-b border-[#D4AF37]/15'
            : scrolled 
              ? 'py-2.5 bg-white/95 backdrop-blur-md shadow-clean border-b border-[#E8E2D5] font-mono' 
              : 'py-3 sm:py-4 bg-[#FAF8F5]/90 backdrop-blur-sm border-b border-[#E8E2D5]/60 font-mono'
        }`}
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 sm:gap-4">
          ...
          <a href="#" className="flex items-center gap-2 sm:gap-3 group shrink-0">
            ...
            <div className="leading-tight">
              <span className={`text-sm sm:text-base lg:text-lg font-bold tracking-wider block whitespace-nowrap ${
                isDark ? 'font-cinzel text-gold-gradient' : 'font-mono text-stone-900'
              }`}>
                SHUBHAARAMBH
              </span>
              <span className={`text-[8px] sm:text-[9px] lg:text-[10px] font-medium tracking-[0.16em] block uppercase whitespace-nowrap ${
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 xl:gap-3 shrink-0">
            ...
            <button
              ...
              className={`p-2 sm:px-3 sm:py-1.5 rounded-full flex items-center gap-1.5 text-xs font-semibold transition-all duration-200 active:scale-95 border ${
                isDark
            ...
            >
              {isDark ? (
                ...
                <>
                  <Moon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-stone-700" />
            ...
            <button
              ...
              className={`px-3 sm:px-4 xl:px-5 py-1.5 sm:py-2 rounded-full font-medium text-[11px] sm:text-xs tracking-wider uppercase transition-all duration-300 flex items-center gap-1.5 whitespace-nowrap active:scale-95 ${
                isDark
            ...
            <button
              ...
              className={`lg:hidden p-2 rounded-xl transition-colors focus:outline-none ${
                isDark ? 'text-slate-200 hover:bg-slate-800/60' : 'text-stone-800 hover:bg-stone-200/60'
              }`}
      ...
      {mobileMenuOpen && (
        <div 
          className={`lg:hidden fixed inset-0 top-[66px] sm:top-[74px] z-40 backdrop-blur-2xl border-t flex flex-col justify-between p-5 pb-20 overflow-y-auto animate-fadeIn ${
            isDark 
